...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            self.out_width = (self.out_width-kernel_sizes[i])//strides[i]+1
        # import pdb; pdb.set_trace()
        self.out_width = self.out_width*num_channels[-1]
        self.convolutional_layers = []
        self.convolutional_layers.append(Conv1D(num_input_channels, num_channels[0],kernel_sizes[0],strides[0], weight_init_fn = conv_weight_init_fn, bias_init_fn=bias_init_fn))
        self.activations = activations
        for i in range(self.nlayers-1):
            self.convolutional_layers.append(Conv1D(num_channels[i],num_channels[i+1],kernel_sizes[i+1],strides[i+1],weight_init_fn = conv_weight_init_fn, bias_init_fn=bias_init_fn))
        self.flatten = Flatten()
        self.linear_layer = Linear(self.out_width, num_linear_neurons,linear_weight_init_fn, bias_init_fn)
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers):
            l = self.nlayers-1-i
            grad = self.activations[l].derivative()*grad
            grad = self.convolutional_layers[l].backward(grad)
