...
class PyramidLSTM(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        length = x.size(0)
        batch_size = x.size(1)
        output, _ = self.pblstm(x)  # (T, B, D*2)
        if length % 2 is not 0:
            padding = torch.zeros(1, batch_size, self.hidden_dim*2).to(DEVICE)
            output = torch.cat((output, padding), dim=0)
        new_length = int(output.size(0)/2)
        output = output.transpose(0, 1)  # (B, T, D*2)
        output = output.reshape(batch_size, new_length, self.hidden_dim*4)  # (B, T/2, D*4)
        output = output.transpose(0, 1)  # (T/2, B, D*4)
        assert output.size(0)*2 - length <= 1, "concatenate dimension error"
        ...
        output = utils.rnn.pack_padded_sequence(output, lengths, enforce_sorted=False)

        return output, lengths


class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, pyramid_layer, value_size=128, key_size=128, bidirectional=True):
        ...
        for i in range(self.pyramid_layer):
            if i == 0:
                self.pblstm.append(PyramidLSTM(self.hidden_dims*2, self.hidden_dims))
            else:
                self.pblstm.append(PyramidLSTM(self.hidden_dims*4, self.hidden_dims))
        self.pblstm = nn.ModuleList(self.pblstm)

        self.value_network = nn.Linear(self.hidden_dims*4, self.value_size)
        self.key_network = nn.Linear(self.hidden_dims*4, self.key_size)
    ...
    def forward(self, x, lengths):
        ...
        outputs, _ = self.blstm(packed_x)  # (T, B, 2*D)

        for pblstm_layer in self.pblstm:
            # print(lengths)
            outputs, lengths = pblstm_layer(outputs, lengths)

        # import pdb; pdb.set_trace()
        # print(lengths)
        linear_input, _ = utils.rnn.pad_packed_sequence(outputs)  # (T, B, D*4)
        # print(linear_input.size())
        linear_input = linear_input.transpose(0, 1)  # (B, T, D*4)
...
class Attention(nn.Module):
    ...
    def forward(self, key, value, query, lens):
        """
        :param key:  (B, Max_len, key_size), Key projection of encoder
        :param value: (B, Max_len, value_size), Value projection of encoder
        ...
        max_len = key.size(1)
        energy = torch.bmm(key, query.unsqueeze(2)).squeeze(2)  # (B, T)
        attention_mask = torch.arange(max_len).unsqueeze(0) >= torch.tensor(lens).unsqueeze(1)
        attention_mask = attention_mask.to(DEVICE)
        energy.masked_fill_(attention_mask, -1e9)
        attention = nn.functional.softmax(energy, dim=1)  # (B, T)
        attention_context = torch.bmm(attention.unsqueeze(1), value).squeeze(1)  # (B, value_size)

        return attention_context, attention_mask


class Decoder(nn.Module):
    def __init__(self, vocab_size, hidden_dim, value_size, key_size, use_attention=False):
        ...
        self.lstm1 = nn.LSTMCell(self.hidden_dim + self.value_size, self.hidden_dim)
        self.lstm2 = nn.LSTMCell(self.hidden_dim, self.key_size)

        if self.use_attention:
            self.attention = Attention()

        self.linear = nn.Sequential(
            nn.Linear(self.key_size + self.value_size, self.hidden_dim),
            nn.Tanh(),
            nn.Linear(self.hidden_dim, self.vocab_size)
        )

    def forward(self, keys, values, lens, text=None, istrain=True):
        """
        :param keys: (B, max_len, key_size) output of the encoder key projection
        :param values: (B, max_len, value_size) output of the encoder value projection
        ...
        if istrain:
            max_len = text.size(1)
            embeddings = self.embedding(text)  # (B, max_len, hidden_dim)
        else:
            max_len = 250
        ...
        for i in range(max_len):
            ...
            hidden_states[1] = self.lstm2(input2, hidden_states[1])  # (B, key_size)

            lstm_outputs = hidden_states[1][0]
            if self.use_attention:
                context, mask = self.attention(keys, values, lstm_outputs, lens)

            linear_input = torch.cat((lstm_outputs, context), dim=1)
            prediction = self.linear(linear_input)  # (batch, vocab)
            ...
            if istrain:
                ...
                # import pdb
                # pdb.set_trace()
                predictions.append(prediction.argmax(dim=-1).unsqueeze(1))
                # if prediction.argmax(dim=-1) == 34:
                #     infer_lens.append(i+1)
                #     predictions.append(torch.zeros(batch_size, max_len-i))
                #     break

        # if not istrain:
        #     return torch.cat(predictions, dim=1), infer_lens
        return torch.cat(predictions, dim=1)


class Seq2Seq(nn.Module):
    def __init__(self, input_dim, vocab_size, hidden_dim, value_size, key_size, pyramidlayers=3, useattention=True):
        ...
        self.encoder = Encoder(self.input_dim, self.hidden_dim, self.pyramidlayers, self.value_size, self.key_size)
        self.decoder = Decoder(self.vocab_size, self.hidden_dim, self.value_size, self.key_size, self.useattention)

    def forward(self, speech_input, speech_lens, text_input=None, istrain=True):

        key, value, lens = self.encoder(speech_input, speech_lens)
        if istrain:
            predictions = self.decoder(keys=key, values=value, lens=lens, text=text_input, istrain=True)
        else:
            predictions = self.decoder(keys=key, values=value, lens=lens, text=None, istrain=False)
